...
$head='';
$recordId =isset($_GET['id']) ? $_GET['id'] : (isset($_POST['id']) ? $_POST['id'] : null); // record id from URL or post data
        ...
        <div class="inner">
            ...
            ?>
                ...
                if ($stmt_select = mysqli_prepare($dbc, "SELECT t1.username, t2.date, t2.comment FROM mybloguser as t1, commenttable as t2 WHERE t2.postid=?  AND t2.userid = t1.userid order by t2.date desc"))
                {
                    ...
                    {
                        trigger_error(mysqli_stmt_error($stmt_select), E_USER_WARNING);
                        ?><div class="bg-danger">Комментарии не найдены</div><?php
                    } else
                    {
                ...
                if (isset($_SESSION['user_id']))
                {
                    if (!empty($errors['_form'])) { ?>
                        <div class="bg-danger"><?php echo $errors['_form'] ?></div>
                    <?php } ?>
                    <!-- New commit -->
                    ...
                    <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                        ...
                        <?php if (!empty($errors['commit'])) { ?>
                            <div class="bg-danger"><?php echo $errors['commit'] ?></div>
                        <?php } ?>
                        ...
                        <?php if (!empty($errors['verify'])) { ?>
                            <div class="bg-danger"><?php echo $errors['verify'] ?></div>
                        <?php } ?>
